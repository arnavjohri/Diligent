...
| A13 | §3 step 6 | A11 and A12: background `SUBMIT` of `ZSD_CUSTOMER_DATA` with ALV capture and runtime field resolution | **Supersedes A11 and A12.** Direct `SELECT` of `KUNNR`, `LCATEGORY`, `LNAME` from `ZSD_CUSTEMP_ASSG` for `GT_PARTNER`, levels `L4`/`L5`/`L6`, `STARTVAL <= SY-DATUM <= ENDVAL`; `LNAME` copied by level; M18 when nothing is maintained; `TY_CUST`/`TY_OUTPUT` level fields typed off `ZSD_CUSTEMP_ASSG-LNAME`; SUBMIT, capture, `F_HIER_FIELD` and `GC_HIER_*` commented out | the callee's source (17/09/26) shows its L4/L5/L6 are that table read, fields `LNAME4`–`LNAME6`; same result without a 3600-line report, its authorisation check and an ALV capture per run (ISSUES.md #27) |
