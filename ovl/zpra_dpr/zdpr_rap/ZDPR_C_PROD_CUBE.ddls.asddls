...

{
  ...
  /* ── OVL share of the BOE total: BoepdQty × PI / 100, signed ──────────── */
  /* Unit-free measure so the detail app (Analytical List Page on
     ZDPR_Q_PROD_QUERY) can chart Oil and Gas on one axis; every other
     quantity measure carries a per-product unit and the chart refuses
     mixed units. Same conversion as BoepdQty, same PI share as OvlShareQty1. */
  @EndUserText.label: 'OVL Share (BOE)'
  @Aggregation.default: #SUM
  cast(
    ( case Daily.Product
        when '722000004' then
