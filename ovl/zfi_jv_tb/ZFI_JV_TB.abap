...
*       AND rrecin IN  p_rrecin.
  ...
  FROM jv_jvto1_acdoca_4a_4c_switch
  INTO TABLE @lt_alljv
  WHERE ryear  =  @p_year
    AND rbukrs =  @p_bukrs
    AND rjvnam IN @p_jvnam
*BOC By SAP_ABAP on 14/09/26
*    AND rldnr  =  '4A'
    AND rldnr  IN ( '4A', '4C' )
*EOC By SAP_ABAP on 14/09/26
    AND rrcty  =  '0'
    AND rrecin IN @p_rrecin.
...
*BOC By SAP_ABAP on 14/09/26
* Issue: opening balance (HSLVT) blank since the switch from JVTO1 to
* JV_JVTO1_ACDOCA_4A_4C_SWITCH. JV data on this landscape spans ledgers 4A
* and 4C - ZOCV_OVL_TRANSFER_F01 (f_read_jvso1) reads both, and the view's
* own _4A_4C_SWITCH name says the same. RLDNR = '4A' alone therefore drops
* the carry-forward rows. Every read of the totals view now takes both
* ledgers (AT SELECTION-SCREEN venture list, account list, 3 balance reads).
* ASSUMPTION: 4A and 4C are SPLIT ledgers (each posting sits in one). If
* they are PARALLEL (same postings in both) this DOUBLES the opening balance
* and the fix is to read 4C only. Verify: SE16 on the view, one venture +
* one BS account, no ledger filter - same amount under both = parallel.
* The JV_JVSO1_ACDOCA line-item reads stay on 4A on purpose: Dr/Cr were
* correct before this change and are not part of the issue.
*EOC By SAP_ABAP on 14/09/26
...
*    GROUP BY rjvnam racct.
  ...
  INTO CORRESPONDING FIELDS OF TABLE @lt_jvso1_1
  WHERE ryear  =  @p_year
    AND rbukrs =  @p_bukrs
    AND rjvnam IN @p_jvnam
    AND poper  BETWEEN @s_period-low AND @s_period-high
...
SELECT SUM( hsl ) AS hsl,
       ...
       racct
  FROM jv_jvso1_acdoca
  INTO CORRESPONDING FIELDS OF TABLE @lt_jvso1_2
  WHERE ryear  =  @p_year
    AND rbukrs =  @p_bukrs
    AND rjvnam IN @p_jvnam
    AND poper  BETWEEN @s_period-low AND @s_period-high
...
* view conversion dropped KSLVT. Restored, and ledger widened as above.
*SELECT SUM( hslvt ) AS hslvt,
*       rjvnam,
*       racct
*  FROM jv_jvto1_acdoca_4a_4c_switch
...
*--------------------------------------------------------------*
FORM disp_data .
******************************************Added on 01.11.2022 by hrishin*****************************
